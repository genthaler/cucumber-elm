...
indentation cx p =
    let
        level =
            Maybe.withDefault 0 (List.head cx)
...
compoundStmt cx =
    let
        parsers =
            [ whileStmt, forStmt, withStmt, funcStmt ]
    in
        choice <| List.map (\p -> blockStmt cx p) parsers


stmt : Ctx -> Parser ( Ctx, C )
stmt cx =
    compoundStmt cx `or` simpleStmt cx
